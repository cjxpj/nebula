...
func (s *ServeRouter) NapCatBOTLoadDicFuncs(dic *Dic) {
	...
	dic.SetFunc("戳一戳", func(val *dto.DicVal, inputs *utils.DicInputs) (any, error) {
		if !inputs.LenOk("1|2") {
			return nil, errors.New("参数错误[1|2]")
		}
		if inputs.LenOk("2") {
			n.Nudge(inputs.Int64(1), inputs.Int64(2))
		} else {
			n.NudgePrivate(inputs.Int64(1))
		}
		return "", nil
	})
	...
	dic.SetFunc("发送音乐卡片", func(val *dto.DicVal, inputs *utils.DicInputs) (any, error) {
		if !inputs.LenOk("5") {
			return nil, errors.New("参数错误[5]")
		}
		n.SendGroupMusic(inputs.Int64(1), inputs.String(2), inputs.String(3), inputs.String(4), inputs.String(5))
		return "", nil
	})

	dic.SetFunc("私聊", func(val *dto.DicVal, inputs *utils.DicInputs) (any, error) {
		if !inputs.LenOk("2") {
			return nil, errors.New("参数错误[2]")
		}
		rMsg := inputs.String(2)
		if rMsg != "" {
			rMsg = strings.ReplaceAll(rMsg, "\\r", "\n")
			n.SendPrivateText(inputs.Int64(1), rMsg)
		}
		return "", nil
	})
...
// 点赞处理
func (s *ServeRouter) NapCatBOTProfileLikeRun(msgData *napcatbottool.MessagePayload) {
	botDicPath := utils.NewFileQueue(s.NapCatBot.FilePath + "/dic")
	botDicList, err := botDicPath.GetFileList()
	if err != nil {
		return
		...
		go func() {
			...
			dic.SetFunc("调用", func(val *dto.DicVal, inputs *utils.DicInputs) (any, error) {
				...
				go func() {
					...
					if rMsg != "" {
						rMsg = strings.ReplaceAll(rMsg, "\\r", "\n")
						s.NapCatBot.SendPrivateText(userID, rMsg)
					}
				}()
				return "", nil
			})
			...
			rMsg := dic.RunPrivate(fmt.Sprintf("点赞 %d", msgData.Times))
			if rMsg != "" {
				rMsg = strings.ReplaceAll(rMsg, "\\r", "\n")
				body, err := s.NapCatBot.SendPrivateText(userID, rMsg)
				if err != nil {
					fmt.Println(err)
				}
				fmt.Println(string(body))
...
// 私聊消息处理
func (s *ServeRouter) NapCatBOTPrivateRun(msgData *napcatbottool.MessagePayload) {
		...
		go func() {
			...
			dic.SetFunc("调用", func(val *dto.DicVal, inputs *utils.DicInputs) (any, error) {
				...
				go func() {
					...
					if rMsg != "" {
						rMsg = strings.ReplaceAll(rMsg, "\\r", "\n")
						s.NapCatBot.SendGroupText(userID, rMsg)
					}
				}()
				return "", nil
			})
...
func (s *ServeRouter) NapCatBotRun(w http.ResponseWriter, r *http.Request) {
	...
	if payload.SubType == "profile_like" {
		s.NapCatBOTProfileLikeRun(payload)
		w.Write([]byte(`{}`))
		return
	}
