...
type DicRunner interface {
	...
	// 未命中时只跑头部（初始化代码）、不跑正文，也不会把触发词行当文本输出。
	// fellBack=true 表示触发词未命中任何词条，调用方据此提示「触发词未命中」。
	DicRunScript(D *dic_dto.Dic, trigger string, timeout time.Duration) (result string, timedOut bool, fellBack bool)
}
