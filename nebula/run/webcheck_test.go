...
<p>{{.版本}}</p>
<p>{{.缺失键}}</p>
<script type="nebula" id="版本">%版本%</script>
<script type="nebula">
图片数据:$某函数 1$
</script>
</body></html>`
	...
	if got := lineOf("模板键不存在：缺失键"); got != 3 {
		t.Fatalf("模板键告警行号 = %d，期望 3", got)
	}
	if got := lineOf("变量未使用：图片数据"); got != 6 {
		t.Fatalf("变量未使用告警行号 = %d，期望 6", got)
	}
	if containsText(warnings, "模板键不存在：版本") {
		t.Fatalf("脚本块 id 本身就是模板键，不应告警：%v", warningsText(warnings))
	}
	if len(warnings) != 2 {
		t.Fatalf("期望 2 条告警，实际：%v", warningsText(warnings))
	}
}

// TestWebDicCheckInlineBlock <?n ... ?> 内联块与脚本块一样算执行块：
// 块里的语句不报「脚本块外的 Nebula 语法」，块内赋值的变量并入模板键。
...
<p>{{.甲}}</p>
	...
	if warnings := WebDicCheck(text); len(warnings) != 0 {
		t.Fatalf("内联块里的语句不应有告警，实际：%v", warningsText(warnings))
	}
	...
	if len(keys) != 1 || keys[0] != "甲" {
		t.Fatalf("模板键 = %v，期望 [甲]", keys)
	}
}
...
乙:2
	...
	if len(keys) != 3 {
		t.Fatalf("模板键 = %v，期望 3 个", keys)
	}
	for _, k := range []string{"版本", "甲", "乙"} {
		if !got[k] {
