...
// aiBuiltinAgents 出厂内置的智能体：首个是覆盖三类词库场景的「综合词库」，其余三个按词库类型
// 划分、各负责一类场景；每个都声明自己可用的内置技能（与 aiBuiltinSkills 对应）。
...
// 智能体（列表首个，即覆盖三类场景的「综合词库」）。只读配置、不持锁，供不持有 aiAgentsMu 的调用方（会话归一化）使用；
