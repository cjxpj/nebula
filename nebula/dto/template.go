...
func compileVarSegments(s string) []varSegment {
	...
	for {
		...
		close += open + 1
		if open > start {
			segs = append(segs, varSegment{isVar: false, text: s[start:open]})
		}
		segs = append(segs, varSegment{isVar: true, text: s[open+1 : close]})
		start = close + 1
	}
	if start < len(s) {
...
// 若某变量段求值结果为非字符串，则整个模板返回该值（与 replaceProcessedContent 行为一致）。
