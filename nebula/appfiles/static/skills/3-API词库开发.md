...
2. 所以 API 词库必须写 Main（与机器人功能词库刚好相反），Main 正文直接输出的内容就是 HTTP 响应体；
...
4. .n 与 .wn 的区别：路径为 .wn 时走网页词库（HTML + 脚本块，无触发词），路径为 .n 时走本条 Main 机制。
...
2. 函数名、参数个数与用法一律以内置文档（read_dic_doc，按需读取对应篇章）为准，不确定就查证或向用户确认，不要写试探词条。
