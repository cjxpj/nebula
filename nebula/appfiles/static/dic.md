...
1. **连接超时**：WebSocket 连接超时时间为 10 秒
2. **重连机制**：连接断开后会自动尝试重连（每 3 秒一次）
4. **词库编码**：词库文件需使用 UTF-8 编码
5. **换行符**：消息内容中的 `\r` 会被转换为换行符